from dataloading.dataloader import get_players_dataset
...
def train():
    ...
    class_weights = torch.tensor(
        [(total_samples / class_counts[cls]) ** alpha for cls in range(len(class_counts))],
        dtype=torch.float32
    ...
    for epoch in range(start_epoch+1,num_epochs):
        ...
        for i,(images,labels) in enumerate(train_loader):
            if i%40==0 and i!=0:
                logger.info(f" Iteration {i/len(train_loader)} Loss {train_loss/i*64} Accuracy {train_correct/(i*64)}")
        ...
        logger.info(f"Training Epoch {epoch} : Loss {train_loss/len(train_dataset)} Accuracy {train_correct/len(train_dataset)}")    
        ...
        with torch.no_grad():
            ...
            for idx,(images,labels) in enumerate(test_loader):
                if idx%40==0 and idx!=0:
                    logger.info(f"Validation Iteration {idx/len(test_loader)} Loss {test_loss/idx*64} Accuracy {test_correct/(idx*64)}")
                images=images.to(device)
                labels=labels.to(device)
                outputs=model(images)
            ...
            logger.info(f"Testing Epoch {epoch} : Loss {test_loss/total} Accuracy {test_correct/total}")
        scheduler.step(test_loss)
        save_checkpoint(epoch,model.state_dict(),optimizer.state_dict(),train_loss/len(train_dataset),test_loss/len(test_dataset),test_correct/total,"models_trained/player_level_classification/checkpoints","models_trained/player_level_classification/best_model.pth")
